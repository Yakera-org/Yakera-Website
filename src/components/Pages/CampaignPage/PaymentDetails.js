...
class PaymentDetails extends PureComponent {
  ...
  validateData() {
    let emptyWarning = this.props.EN
      ? "This field cannot be empty"
      : "Este campo no puede estar vacío";
    let amountError, emailError, nameError, tipError;

    let isValid = false;
    tipError = validateFields.validateNumberIncludeZero(
      this.state.tip.value + ""
    );
    ...
    if (!this.state.amount.value) {
      ...
      amountError = validateFields.validateNumberForAmount(
        this.state.amount.value + ""
      );
    }
    ...
    if (!this.state.age || !this.state.consent) {
      isValid = false;
      this.setState({
        checkError: this.props.EN
          ? "These checkboxes must be ticked."
          : "Estas casillas de verificación deben estar marcadas.",
      });
      ...
      <div>
        ...
        <Grid container spacing={2} style={{ alignItems: "flex-start" }}>
          <Grid item xs={12} sm={6}>
            <div className="option">{EN ? "Name" : "Nombre"}</div>
          ...
          <Grid item xs={12} sm={6}>
            <div className="option">{EN ? "Email" : "Correo electrónico"}</div>
        ...
        <div className="align-left">
          <label className="checkbox-button">
            <input
              ...
              className={classnames("form-control", "checkbox-square")}
            />
