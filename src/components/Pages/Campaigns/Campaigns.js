...
function Campaigns() {
  ...
  async function LoadCampaignsFromBackend(
    page,
    category,
    isNewCat,
    dateOrder,
    ...
    try {
      ...
      const payload = `/campaigns/?${pageLimit ? `page=${newPageNum}` : ""}${
        pageLimit ? `&limit=${pageLimit}` : ""
      }${filter === "percent" ? `&percentage=desc` : ""}${
        filter === "raised" ? `&raised=desc` : ""
      }${filter === "date" ? `&sort=${dateOrder}` : ""}${
        filter === "" ? `&sort=desc` : ""
      }${category ? `&category=${category}` : ""}`; //has to be one line string
  ...
  function getCampaigns(data, filter, page, query) {
    ...
    const extractedCampaigns = !query
      ? allCampaigns
      : extractCampaigns(allCampaigns, query);
    if (filter !== "percent" && filter !== "raised" && !query)
      return extractedCampaigns; //already paginated
    const _currentPage = page - 1;
    const campaignExtract = extractedCampaigns.slice(
      NUM_OF_ITEMS_PER_PAGE * _currentPage,
      NUM_OF_ITEMS_PER_PAGE * (_currentPage + 1)
    );
  ...
  function extractCampaigns(campaigns, query) {
    var filteredCampaigns = campaigns.filter((campaign) => {
      let campaignTitles = campaign.translations["en"]
        ? campaign.translations["en"].title.toLowerCase()
        : "";
      campaignTitles += campaign.translations["es"]
        ? campaign.translations["es"].title.toLowerCase()
        : "";
  ...
  function getPageNum(data, filter, query) {
    if (filter !== "percent" && filter !== "raised" && !query)
      return data.pages; //backend already returns total number of pages
    const camNum = !query
      ? data.data.campaigns.length
      : extractCampaigns(data.data.campaigns, query).length;
    const pageNum = Math.ceil(camNum / NUM_OF_ITEMS_PER_PAGE);
    return pageNum;
  }

  function getPageLimit(filter, query) {
    return filter === "percent" || filter === "raised" || query
      ? ""
