...
function HighlightedCampaign(props) {
  ...
  const campaign = {
    ...
    donationName:
      noNullDonations[numberOfDonations - 1]?.name?.split(" ")[0] || "",
    ...
    >
      <Grid item xs={6} sm={6} className="header-mid-img">
        <div className="image-top">
          {EN ? (
            <>
              Campaign of the <b>week</b>
            </>
          ) : (
            <>
              Campaña de la <b>Semana</b>
            </>
          )}
        </div>
        <img
          ...
          onClick={() => (window.location = `/campaign/${campaign.slug}`)}
        />
      </Grid>
      ...
      <Grid item xs={6} sm={6} className="mid-content">
        ...
        <div className="bubble-text">
          <div className="bubble-quote">
            <img
              alt="donate-figure"
              src="https://assets.yakera.org/yakera/profile-icon-1.webp"
            />
            <div id="text">
              <b> {campaign.anon ? "Anonymous" : campaign.donationName} </b>{" "}
              <br />
              {campaign.donationComment
                ? '"' + campaign.donationComment + '"'
                : EN
                ? "Most recent donor"
                : "Donante más reciente"}
            </div>
          </div>
          <div className="bubble-digit">${campaign.donationAmount} USD</div>
        </div>
      </Grid>
    </Grid>
  );
